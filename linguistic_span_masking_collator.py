...
def tokenize_with_offsets(examples, tokenizer, max_length: int = 128):
    ...
    """
    # First, get the basic tokenization
    encoding = tokenizer(
        examples["text"],
        truncation=True,
    ...
    # Create special_tokens_mask manually to ensure it's always present
    # Collect all special token IDs from the tokenizer
    special_token_ids = set()
    if hasattr(tokenizer, 'all_special_ids'):
        special_token_ids = set(tokenizer.all_special_ids)
    else:
        # Fallback: manually add known BERT special tokens
        for attr in ['cls_token_id', 'sep_token_id', 'pad_token_id', 'unk_token_id', 'mask_token_id']:
            token_id = getattr(tokenizer, attr, None)
            if token_id is not None:
                special_token_ids.add(token_id)
    
    # Create mask: 1 for special tokens, 0 for regular tokens
    special_tokens_mask = []
    for input_ids in encoding["input_ids"]:
        mask = [1 if token_id in special_token_ids else 0 for token_id in input_ids]
        special_tokens_mask.append(mask)
    
    encoding["special_tokens_mask"] = special_tokens_mask
...
class LinguisticSpanMaskingDataCollator:
    ...
    def __call__(
        ...
        Collate a batch. Each feature dict must contain:
          ...
          - special_tokens_mask  : List[int]  (or will be created)
          - offset_mapping       : List[Tuple[int,int]]
        ...
        # Ensure all features have special_tokens_mask; create it if missing
        special_token_ids = set()
        ...
        for f in features:
            if "special_tokens_mask" not in f:
                # Create special_tokens_mask: 1 for special tokens, 0 for regular
                input_ids = f.get("input_ids", [])
                f["special_tokens_mask"] = [
                    1 if token_id in special_token_ids else 0 for token_id in input_ids
                ]
